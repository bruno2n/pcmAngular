import { DadosEquipamentosComponent } from './dados-equipamentos/dados-equipamentos.component';
...
export class EquipamentosComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this._array1.map(e=>{
            this.crudService.getAll(`FROTA/${e}/`).subscribe((res: any[]) => {  
              this.dados = res
      console.log(res)
    ...
    this.carregarDados(this.inputZona)
        
}

  carregarDados(inputZona: string) {    
    this.dados = []
    this.crudService.getAll(`FROTA/${inputZona}/`).subscribe((res: any[]) => {            
      this.dados = res
      console.log(res)
      this._length = Object.keys(res).length
